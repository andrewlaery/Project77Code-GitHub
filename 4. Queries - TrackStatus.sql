...
DROP TABLE IF EXISTS TTX_MEMBER_TMIQUALIFICATIONS0;
CREATE TABLE TTX_MEMBER_TMIQUALIFICATIONS0 AS
  SELECT
  	-- NameFull ,
      Qualification ,
      QualificationStatus
...
DROP TABLE IF EXISTS TTX_MEMBER_MOSTRECENTPROJECT0;
CREATE TABLE TTX_MEMBER_MOSTRECENTPROJECT0 AS
  SELECT
      MembersID ,
      NameFull ,
      Track ,
...
DROP TABLE IF EXISTS TTX_MEMBER_MOSTRECENTPROJECT1;
CREATE TABLE TTX_MEMBER_MOSTRECENTPROJECT1 AS
  SELECT
    Qualification ,
    Manual ,
    Project ,
...
CREATE TABLE TTX_CC_PROJECT_STATUS_PCENT0 AS
	SELECT
        ...
        (SELECT COUNT(*) FROM TTX_MEMBER_MOSTRECENTPROJECT0 WHERE NameFull = @NameFull AND QualificationsID = '1' AND QualificationStatus = 'In progress') / '10' AS Percentage;
