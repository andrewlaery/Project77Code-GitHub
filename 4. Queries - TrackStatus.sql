...
DROP TEMPORARY TABLE IF EXISTS TTX_MEMBER_TMIQUALIFICATIONS0;
CREATE TEMPORARY TABLE TTX_MEMBER_TMIQUALIFICATIONS0 AS
  SELECT
  	-- NameFull ,
      Qualification ,
      QualificationStatus
...
SELECT * FROM TTX_MEMBER_TMIQUALIFICATIONS0;

-- Creates a view of all most recent projects. Could refer directly to the underlying table. ALL MEMBERS, ALL QUALIFICATIONS.
DROP TEMPORARY TABLE IF EXISTS TTX_MEMBER_MOSTRECENTPROJECT0;
CREATE TEMPORARY TABLE TTX_MEMBER_MOSTRECENTPROJECT0 AS
  SELECT
      MembersID ,
      NameFull ,
      Track ,
...
-- SELECT * FROM TTX_MEMBER_MOSTRECENTPROJECT0;

DROP TEMPORARY TABLE IF EXISTS TTX_MEMBER_MOSTRECENTPROJECT1;
CREATE TEMPORARY TABLE TTX_MEMBER_MOSTRECENTPROJECT1 AS
  SELECT
    Qualification ,
    Manual ,
    Project ,
    Role ,
    Date1
  FROM TTX_MEMBER_MOSTRECENTPROJECT0
  WHERE
    NameFull = @NameFull
...
SELECT * FROM TTX_MEMBER_MOSTRECENTPROJECT1;
...
DROP TEMPORARY TABLE IF EXISTS TTX_CC_PROJECT_STATUS0;
CREATE TEMPORARY TABLE TTX_CC_PROJECT_STATUS0 AS
  SELECT *
  FROM TTX_MEMBER_MOSTRECENTPROJECT0
  WHERE
    NameFull = @NameFull
...
-- Setup table
DROP TEMPORARY TABLE IF EXISTS TTX_CC_PROJECT_STATUS_PCENT0_SETUP1;
CREATE TEMPORARY TABLE TTX_CC_PROJECT_STATUS_PCENT0_SETUP1 AS
	SELECT
		(SELECT QualificationsID FROM TTX_MEMBER_MOSTRECENTPROJECT0 WHERE QualificationsID = '1' GROUP BY QualificationsID) AS QualificationsID;
-- SELECT * FROM TTX_CC_PROJECT_STATUS_PCENT0_SETUP1;

-- Setup table
DROP TEMPORARY TABLE IF EXISTS TTX_CC_PROJECT_STATUS_PCENT0_SETUP2;
CREATE TEMPORARY TABLE TTX_CC_PROJECT_STATUS_PCENT0_SETUP2 AS
	SELECT
		(SELECT Qualification FROM TTX_MEMBER_MOSTRECENTPROJECT0 WHERE QualificationsID = '1' GROUP BY Qualification) AS Qualification ;
-- SELECT * FROM TTX_CC_PROJECT_STATUS_PCENT0_SETUP2;

-- Setup table
DROP TEMPORARY TABLE IF EXISTS TTX_CC_PROJECT_STATUS_PCENT0_SETUP3;
CREATE TEMPORARY TABLE TTX_CC_PROJECT_STATUS_PCENT0_SETUP3 AS
	SELECT
		(SELECT COUNT(*)/10 FROM TTX_MEMBER_MOSTRECENTPROJECT0 WHERE NameFull = @NameFull AND QualificationsID = '1' AND QualificationStatus = 'In progress') AS Percentage;
-- SELECT * FROM TTX_CC_PROJECT_STATUS_PCENT0_SETUP3;

-- Aggregate table from above queries
DROP TEMPORARY TABLE IF EXISTS TTX_CC_PROJECT_STATUS_PCENT0;
CREATE TEMPORARY TABLE TTX_CC_PROJECT_STATUS_PCENT0 AS
	SELECT
			TTX_CC_PROJECT_STATUS_PCENT0_SETUP1.QualificationsID ,
			TTX_CC_PROJECT_STATUS_PCENT0_SETUP2.Qualification ,
			TTX_CC_PROJECT_STATUS_PCENT0_SETUP3.Percentage
	FROM TTX_CC_PROJECT_STATUS_PCENT0_SETUP1
	CROSS JOIN TTX_CC_PROJECT_STATUS_PCENT0_SETUP2
	CROSS JOIN TTX_CC_PROJECT_STATUS_PCENT0_SETUP3;
...
-- Setup table
DROP TEMPORARY TABLE IF EXISTS TTX_ACB_CC_STATUS_SETUP1;
CREATE TEMPORARY TABLE TTX_ACB_CC_STATUS_SETUP1 AS
	SELECT
		(SELECT QualificationsID FROM TTX_QUALIFICATIONS0 WHERE QualificationsID = '1'  GROUP BY QualificationsID) AS QualificationsID;
-- SELECT * FROM TTX_ACB_CC_STATUS_SETUP1;

-- Setup table
DROP TEMPORARY TABLE IF EXISTS TTX_ACB_CC_STATUS_SETUP2;
CREATE TEMPORARY TABLE TTX_ACB_CC_STATUS_SETUP2 AS
	SELECT
		(SELECT Qualification FROM TTX_QUALIFICATIONS0 WHERE QualificationsID = '1'  GROUP BY QualificationsID) AS Qualification;
-- SELECT * FROM TTX_ACB_CC_STATUS_SETUP2;

-- Setup table
DROP TEMPORARY TABLE IF EXISTS TTX_ACB_CC_STATUS_SETUP3;
CREATE TEMPORARY TABLE TTX_ACB_CC_STATUS_SETUP3 AS
	SELECT
		IF((SELECT COUNT(*) FROM TTX_QUALIFICATIONS0 WHERE QualificationsID = '1' AND QualificationDate > '1900-01-01 00:00:00' AND NameFull = @NameFull) >= '1', '1', '0' ) AS  ProjectComplete;
-- SELECT * FROM TTX_ACB_CC_STATUS_SETUP3;

-- Aggregate table from above queries
DROP TEMPORARY TABLE IF EXISTS TTX_ACB_CC_STATUS0;
CREATE TEMPORARY TABLE TTX_ACB_CC_STATUS0 AS
	SELECT * FROM TTX_ACB_CC_STATUS_SETUP1
  CROSS JOIN TTX_ACB_CC_STATUS_SETUP2
  CROSS JOIN TTX_ACB_CC_STATUS_SETUP3;
-- SELECT * FROM TTX_ACB_CC_STATUS0;

-- Creates a view of the current Advanced Communictor Bronze projects for each member.
DROP TEMPORARY TABLE IF EXISTS TTX_ACB_PROJECT_STATUS0;
CREATE TEMPORARY TABLE TTX_ACB_PROJECT_STATUS0 AS
	SELECT *
	FROM TTX_MEMBER_MOSTRECENTPROJECT0
	WHERE
		NameFull = @NameFull
...
DROP TEMPORARY TABLE IF EXISTS TTX_ACB_AC_MANUAL_STATUS_PCENT1;
CREATE TEMPORARY TABLE TTX_ACB_AC_MANUAL_STATUS_PCENT1 AS
...
-- SELECT * FROM TTX_ACB_AC_MANUAL_STATUS_PCENT1;

-- Combines the manuals to provide a percentage completed for the Advanced Communicator part of the ACB
DROP TEMPORARY TABLE IF EXISTS TTX_ACB_AC_PROJECT_STATUS_PCENT1;
CREATE TEMPORARY TABLE TTX_ACB_AC_PROJECT_STATUS_PCENT1 AS
	SELECT
		'Advanced Communicator' AS Requirement ,
		(SELECT (SUM(Countx) / '2') FROM TTX_ACB_AC_MANUAL_STATUS_PCENT1) AS ProjectsCompleted;
-- SELECT * FROM TTX_ACB_AC_PROJECT_STATUS_PCENT1;

DROP TEMPORARY TABLE IF EXISTS TTX_ACB_SUMMARY0;
CREATE TEMPORARY TABLE TTX_ACB_SUMMARY0 AS
	SELECT Qualification , ProjectComplete FROM TTX_ACB_CC_STATUS0
	UNION
	SELECT * FROM TTX_ACB_AC_PROJECT_STATUS_PCENT1;
SELECT * FROM TTX_ACB_SUMMARY0;
...
DROP TEMPORARY TABLE IF EXISTS TTX_ACS_ACB_STATUS0;
CREATE TEMPORARY TABLE TTX_ACS_ACB_STATUS0 AS
	SELECT
...
DROP TEMPORARY TABLE IF EXISTS TTX_ACS_PROJECT_STATUS0;
CREATE TEMPORARY TABLE TTX_ACS_PROJECT_STATUS0 AS
	SELECT *
	FROM TTX_MEMBER_MOSTRECENTPROJECT0
	WHERE
		NameFull = @NameFull
...
DROP TEMPORARY TABLE IF EXISTS TTX_ACS_AC_PROJECT_STATUS_PCENT0;
CREATE TEMPORARY TABLE TTX_ACS_AC_PROJECT_STATUS_PCENT0 AS
...
DROP TEMPORARY TABLE IF EXISTS TTX_ACS_BSSSCS_PROJECT_STATUS_PCENT0;
CREATE TEMPORARY TABLE TTX_ACS_BSSSCS_PROJECT_STATUS_PCENT0 AS
...
-- Creates a view of the completion of the Advanced Communicator Bronze Qualification from the Qualification Table.
DROP TEMPORARY TABLE IF EXISTS TTX_ACG_ACS_STATUS0;
CREATE TEMPORARY TABLE TTX_ACG_ACS_STATUS0 AS
	SELECT
...
DROP TEMPORARY TABLE IF EXISTS TTX_ACG_PROJECT_STATUS0;
CREATE TEMPORARY TABLE TTX_ACG_PROJECT_STATUS0 AS
	SELECT *
	FROM TTX_MEMBER_MOSTRECENTPROJECT0
	WHERE
		NameFull = @NameFull
...
-- Counts the top 2 AC Manuals and the % complete
DROP TEMPORARY TABLE IF EXISTS TTX_ACG_AC_PROJECT_STATUS_PCENT0;
CREATE TEMPORARY TABLE TTX_ACG_AC_PROJECT_STATUS_PCENT0 AS
...
-- Counts top 2 BSS SCS Manuals and the % complete
DROP TEMPORARY TABLE IF EXISTS TTX_ACG_SCSL_PROJECT_STATUS_PCENT0;
CREATE TEMPORARY TABLE TTX_ACG_SCSL_PROJECT_STATUS_PCENT0 AS
...
DROP TEMPORARY TABLE IF EXISTS TTX_ACG_OTHER_PROJECT_STATUS_PCENT0;
CREATE TEMPORARY TABLE TTX_ACG_OTHER_PROJECT_STATUS_PCENT0 AS
...
DROP TEMPORARY TABLE IF EXISTS TTX_CL_STRUCTURE_MEMBERS0;
CREATE TEMPORARY TABLE TTX_CL_STRUCTURE_MEMBERS0 AS
	SELECT
...
DROP TEMPORARY TABLE IF EXISTS TTX_CL_STATUS0;
CREATE TEMPORARY TABLE TTX_CL_STATUS0 AS
	SELECT
...
DROP TEMPORARY TABLE IF EXISTS TTX_CL_STATUS1;
CREATE TEMPORARY TABLE TTX_CL_STATUS1 AS
  SELECT * FROM TTX_CL_STATUS0
  WHERE NameFull = @NameFull;
-- SELECT * FROM TTX_CL_STATUS1;

-- CL Project 1 query: Complete 3 of 4 projects
DROP TEMPORARY TABLE IF EXISTS TTX_CL_STATUS_PROJECT1;
CREATE TEMPORARY TABLE TTX_CL_STATUS_PROJECT1 AS
	SELECT
...
DROP TEMPORARY TABLE IF EXISTS TTX_CL_STATUS_PROJECT2;
CREATE TEMPORARY TABLE TTX_CL_STATUS_PROJECT2 AS
	SELECT
...
DROP TEMPORARY TABLE IF EXISTS TTX_CL_STATUS_PROJECT3;
CREATE TEMPORARY TABLE TTX_CL_STATUS_PROJECT3 AS
	SELECT
...
DROP TEMPORARY TABLE IF EXISTS TTX_CL_STATUS_PROJECT4;
CREATE TEMPORARY TABLE TTX_CL_STATUS_PROJECT4 AS
	SELECT
...
DROP TEMPORARY TABLE IF EXISTS TTX_CL_STATUS_PROJECT5;
CREATE TEMPORARY TABLE TTX_CL_STATUS_PROJECT5 AS
	SELECT
...
DROP TEMPORARY TABLE IF EXISTS TTX_CL_STATUS_PROJECT6;
CREATE TEMPORARY TABLE TTX_CL_STATUS_PROJECT6 AS
	SELECT
...
DROP TEMPORARY TABLE IF EXISTS TTX_CL_STATUS_PROJECT7;
CREATE TEMPORARY TABLE TTX_CL_STATUS_PROJECT7 AS
	SELECT
...
DROP TEMPORARY TABLE IF EXISTS TTX_CL_STATUS_PROJECT8;
CREATE TEMPORARY TABLE TTX_CL_STATUS_PROJECT8 AS
	SELECT
...
DROP TEMPORARY TABLE IF EXISTS TTX_CL_STATUS_PROJECT9;
CREATE TEMPORARY TABLE TTX_CL_STATUS_PROJECT9 AS
	SELECT
...
DROP TEMPORARY TABLE IF EXISTS TTX_CL_STATUS_PROJECT10;
CREATE TEMPORARY TABLE TTX_CL_STATUS_PROJECT10 AS
	SELECT
...
DROP TEMPORARY TABLE IF EXISTS TTX_CL_STATUS2;
CREATE TEMPORARY TABLE TTX_CL_STATUS2 AS
...
DROP TEMPORARY TABLE IF EXISTS TTX_ALB_CC_STATUS0;
CREATE TEMPORARY TABLE TTX_ALB_CC_STATUS0 AS
	SELECT
		(SELECT QualificationsID FROM TTX_QUALIFICATIONS0 WHERE QualificationsID = '1'  GROUP BY QualificationsID) AS QualificationsID ,
		(SELECT Qualification FROM TTX_QUALIFICATIONS0 WHERE QualificationsID = '1'  GROUP BY QualificationsID) AS Qualification ,
...
DROP TEMPORARY TABLE IF EXISTS TTX_ALB_CL_STATUS0;
CREATE TEMPORARY TABLE TTX_ALB_CL_STATUS0 AS
	SELECT
...
DROP TEMPORARY TABLE IF EXISTS TTX_ALB_PROJECT_STATUS0;
CREATE TEMPORARY TABLE TTX_ALB_PROJECT_STATUS0 AS
	SELECT *
	FROM TTX_MEMBER_MOSTRECENTPROJECT0
	WHERE
		NameFull = @NameFull
...
DROP TEMPORARY TABLE IF EXISTS TTX_ALB_OTHER_PROJECT_STATUS_PCENT0;
CREATE TEMPORARY TABLE TTX_ALB_OTHER_PROJECT_STATUS_PCENT0 AS
...
DROP TEMPORARY TABLE IF EXISTS TTX_ALB_SCSLES_PROJECT_STATUS_PCENT0;
CREATE TEMPORARY TABLE TTX_ALB_SCSLES_PROJECT_STATUS_PCENT0 AS
...
DROP TEMPORARY TABLE IF EXISTS TTX_ALS_ALB_STATUS0;
CREATE TEMPORARY TABLE TTX_ALS_ALB_STATUS0 AS
	SELECT
...
-- Creates a view of all Advanced Leader Bronze projects
DROP TEMPORARY TABLE IF EXISTS TTX_ALS_PROJECT_STATUS0;
CREATE TEMPORARY TABLE TTX_ALS_PROJECT_STATUS0 AS
	SELECT *
	FROM TTX_MEMBER_MOSTRECENTPROJECT0
	WHERE
		NameFull = @NameFull
...
-- Counts if member has completed Club officer, club success, or training?
DROP TEMPORARY TABLE IF EXISTS TTX_ALS_OTHER_PROJECT_STATUS_PCENT0;
CREATE TEMPORARY TABLE TTX_ALS_OTHER_PROJECT_STATUS_PCENT0 AS
...
-- Counts if member has completed 2 projects from SCS or LES ?
DROP TEMPORARY TABLE IF EXISTS TTX_ALS_HPLP_PROJECT_STATUS_PCENT0;
CREATE TEMPORARY TABLE TTX_ALS_HPLP_PROJECT_STATUS_PCENT0 AS
