...
-- Create view of all Competent Communicator speeches.
...
CREATE VIEW V_ACB_CC_STATUS0 AS
	SELECT
		...
		IF((SELECT COUNT(*) FROM V_QUALIFICATIONS0 WHERE QualificationsID = '1' AND QualificationDate > '0' AND NameFull = 'Emma Armitage') >= '1', '1', '0' ) AS  ProjectComplete;
...
CREATE VIEW V_ACS_ACB_STATUS0 AS
	SELECT
		...
		IF((SELECT COUNT(*) FROM V_QUALIFICATIONS0 WHERE QualificationsID = '2' AND QualificationDate > '0' AND NameFull = 'Emma Armitage') >= '1', '1', '0' ) AS  ProjectComplete;
...
CREATE VIEW V_ACG_ACS_STATUS0 AS
	SELECT
		...
		IF((SELECT COUNT(*) FROM V_QUALIFICATIONS0 WHERE QualificationsID = '3' AND QualificationDate > '0' AND NameFull = 'Emma Armitage') >= '1', '1', '0' ) AS  ProjectComplete;
...
CREATE VIEW V_ACG_SCSL_PROJECT_STATUS_PCENT0 AS
  SELECT Manual , COUNT(*) AS Countx FROM V_ACG_PROJECT_STATUS0 WHERE Qualification = 'Advanced Communicator Gold' AND (ManualGroup = 'Success Communication' OR ManualGroup = 'Success Leadership') GROUP BY Manual ORDER BY Countx DESC Limit 2;
...
DROP VIEW IF EXISTS V_ALB_CC_STATUS0;
CREATE VIEW V_ALB_CC_STATUS0 AS
	SELECT
		(SELECT QualificationsID FROM V_QUALIFICATIONS0 WHERE QualificationsID = '1'  GROUP BY QualificationsID) AS QualificationsID ,
		(SELECT Qualification FROM V_QUALIFICATIONS0 WHERE QualificationsID = '1'  GROUP BY QualificationsID) AS Qualification ,
		IF((SELECT COUNT(*) FROM V_QUALIFICATIONS0 WHERE QualificationsID = '1' AND QualificationDate > '0' AND NameFull = 'Emma Armitage') >= '1', '1', '0' ) AS  ProjectComplete;
SELECT * FROM V_ALB_CC_STATUS0;

DROP VIEW IF EXISTS V_ALB_CL_STATUS0;
CREATE VIEW V_ALB_CL_STATUS0 AS
	SELECT
		...
		IF((SELECT COUNT(*) FROM V_QUALIFICATIONS0 WHERE QualificationsID = '5' AND QualificationDate > '0' AND NameFull = 'Emma Armitage') >= '1', '1', '0' ) AS  ProjectComplete;
SELECT * FROM V_ALB_CL_STATUS0;


-- Creates a view of all Advanced Leader Bronze projects
DROP VIEW IF EXISTS V_ALB_PROJECT_STATUS0;
CREATE VIEW V_ALB_PROJECT_STATUS0 AS
	SELECT *
	FROM V_QUALIFICATION_STATUS0
	WHERE
		NameFull = 'Emma Armitage'
		...
		AND QualificationStatus = 'In progress'
	ORDER BY NameFull , QualificationsOrder , ManualGroupsOrder , ManualsOrder , ProjectsOrder;
SELECT * FROM V_ALB_PROJECT_STATUS0;

-- Counts if member has completed Club officer, club success, or training?
DROP VIEW IF EXISTS V_ALB_OTHER_PROJECT_STATUS_PCENT0;
CREATE VIEW V_ALB_OTHER_PROJECT_STATUS_PCENT0 AS
  SELECT Project , COUNT(*) AS Countx FROM V_ALB_PROJECT_STATUS0 WHERE Qualification = 'Advanced Leader Bronze' AND (ProjectsID = '182' OR ProjectsID = '183') GROUP BY Project ORDER BY Countx DESC Limit 2;
SELECT * FROM V_ALB_OTHER_PROJECT_STATUS_PCENT0;

-- Counts if member has completed 2 projects from SCS or LES ?
DROP VIEW IF EXISTS V_ALB_SCSLES_PROJECT_STATUS_PCENT0;
CREATE VIEW V_ALB_SCSLES_PROJECT_STATUS_PCENT0 AS
  SELECT Manual , COUNT(*) AS Countx FROM V_ALB_PROJECT_STATUS0 WHERE Qualification = 'Advanced Leader Bronze' AND (ManualGroup = 'Successful Club Series' OR ManualGroup = 'Leadership Excellence Series') GROUP BY Manual ORDER BY Countx DESC Limit 2;
SELECT * FROM V_ALB_SCSLES_PROJECT_STATUS_PCENT0;

-- Creates a summary UNION table of all the above queries.
SELECT
	Qualification AS Requirement ,
  ProjectComplete AS ProjectsCompleted
  FROM V_ALB_CC_STATUS0
UNION
SELECT
	Qualification AS Requirement ,
  ProjectComplete AS ProjectsCompleted
  FROM V_ALB_CL_STATUS0
UNION
SELECT
...
DROP VIEW IF EXISTS V_ALS_ALB_STATUS0;
CREATE VIEW V_ALS_ALB_STATUS0 AS
	SELECT
		...
		IF((SELECT COUNT(*) FROM V_QUALIFICATIONS0 WHERE QualificationsID = '6' AND QualificationDate > '0' AND NameFull = 'Emma Armitage') >= '1', '1', '0' ) AS  ProjectComplete;
SELECT * FROM V_ALS_ALB_STATUS0;


-- Creates a view of all Advanced Leader Bronze projects
DROP VIEW IF EXISTS V_ALS_PROJECT_STATUS0;
CREATE VIEW V_ALS_PROJECT_STATUS0 AS
	SELECT *
	FROM V_QUALIFICATION_STATUS0
	WHERE
		NameFull = 'Emma Armitage'
		...
		AND QualificationStatus = 'In progress'
	ORDER BY NameFull , QualificationsOrder , ManualGroupsOrder , ManualsOrder , ProjectsOrder;
SELECT * FROM V_ALS_PROJECT_STATUS0;

-- Counts if member has completed Club officer, club success, or training?
DROP VIEW IF EXISTS V_ALS_OTHER_PROJECT_STATUS_PCENT0;
CREATE VIEW V_ALS_OTHER_PROJECT_STATUS_PCENT0 AS
  SELECT Project , COUNT(*) AS Countx FROM V_ALS_PROJECT_STATUS0 WHERE Qualification = 'Advanced Leader Silver' AND (ProjectsID = '184' OR ProjectsID = '185') GROUP BY Project ORDER BY Countx DESC Limit 2;
SELECT * FROM V_ALB_OTHER_PROJECT_STATUS_PCENT0;

-- Counts if member has completed 2 projects from SCS or LES ?
DROP VIEW IF EXISTS V_ALS_HPLP_PROJECT_STATUS_PCENT0;
CREATE VIEW V_ALS_HPLP_PROJECT_STATUS_PCENT0 AS
  SELECT Manual , COUNT(*) AS Countx FROM V_ALS_PROJECT_STATUS0 WHERE Qualification = 'Advanced Leader Silver' AND (ManualGroup = 'High Performance Leadership Program') GROUP BY Manual ORDER BY Countx DESC;
SELECT * FROM V_ALS_HPLP_PROJECT_STATUS_PCENT0;
