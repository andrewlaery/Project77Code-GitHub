...
SELECT * FROM TTX_CURRENTMEMBERS0 ORDER BY currentclubs , currentmember , NameFull;
...
CREATE TEMPORARY TABLE TTX_CONTESTABLE_TMIROLES_TO_MEMBERS0 AS
    SELECT
			* ,
			CONCAT(TTX_CURRENTMEMBERS0.NameFull , ' - ' , TTX_CONTESTABLE_TMIROLES0.Role) AS NameFull_Role1
